...
const bour_mark = (x, y) =>
  `translate3d(${x / 50 + 230}px,${y / 30 - 180}px,0)`;
    ...
    <div>
      ...
      {windowWidth >= 900 ? (
        <div>
          ...
          <div
            className="container_borough bg_container south_bg"
            onMouseMove={({ clientX: x, clientY: y }) =>
              set({ xy: calc(x, y) })
            }
          >
            <animated.img
              className="bour_mark parallax"
              style={{ transform: props.xy.to(bour_mark) }}
              src={IMAGES[11].url}
            />
            <animated.img
              className="tate parallax"
              style={{ transform: props.xy.to(tate_mus) }}
              src={IMAGES[12].url}
            />
            <animated.img
              className="shakesp parallax"
              style={{ transform: props.xy.to(shakes) }}
              src={IMAGES[13].url}
            />
            <animated.img
              className="cathedral parallax"
              style={{ transform: props.xy.to(cathe) }}
              src={IMAGES[14].url}
            />
            <animated.img
              className="imp_mus parallax"
              style={{ transform: props.xy.to(imp_mus) }}
              src={IMAGES[15].url}
            />
            <animated.img
              className="golden parallax"
              style={{ transform: props.xy.to(golden) }}
              src={IMAGES[16].url}
            />
        ...
        <div>
          <div className="container_borough bg_static">
            <div className="image_stack">
              ...
              <img
                className="imperial_static"
                src={IMAGES[15].url}
                alt="impreial war museum"
              />
