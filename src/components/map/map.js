...
const MapCanvas = () => {
  // initialising min and max camera position for camera animation
  const poi = useRef();
  const minCameraPosition = [0, 0, 7];
  const maxCameraPosition = [0, 0, 20];

  const [isCameraAnimationInProgress, setCameraAnimationInProgress] =
    useState(true);
  ...
  return (
    <div className="map_container">
      ...
      <Canvas
        camera={{ position: initialCameraPosition, fov: 75, up: [0, 0, 1] }}
      >
        <Suspense fallback={null}>
          ...
          <Icon
            position={[10, 5, -9.5]}
            url={"../assets/map_images/icon_images/Newham_Icon.png"}
            size={5}
            to="/newham"
          />
          <Icon
            position={[2, -0.5, -9.5]}
            url={"../assets/map_images/icon_images/Southwark_icon.png"}
            size={4.5}
            to="/southwark"
          />
          <Icon
            position={[11.7, -6, -9.5]}
            url={"../assets/map_images/icon_images/Greenwich_Icon.png"}
            size={5}
            to="/greenwich"
          />
          <Icon
            position={[-12, 7.5, -9.5]}
            url={"../assets/map_images/icon_images/Brent_icon.png"}
            size={4.5}
            to="/brent"
          />
...
function CameraAnimation({
  minCameraPosition,
  maxCameraPosition,
  isCameraAnimationInProgress,
  onAnimationComplete,
}) {
  const { camera } = useThree();

  // UseFrame used to animate the camera position if the animation is still in progress
  useFrame(({ clock }) => {
    if (isCameraAnimationInProgress) {
      ...
      newCameraPosition.lerpVectors(
        new THREE.Vector3(...maxCameraPosition),
        new THREE.Vector3(...minCameraPosition),
        t
      );
